...
# V4 version: Complete tool lifecycle management with conversation tools
...
from .conversation_tools import CompactConversationTool, ConversationStatsTool
from .delegation_tools import SubAgentTool, DelegationInfoTool
...
class ToolManager:
    ...
    def initialize(self):
        ...
        # Conversation management tools (will be initialized with agent instance later)
        self._register_category("Conversation Management", [
            CompactConversationTool(),
            ConversationStatsTool()
        ])

        # Delegation tools (will be initialized with agent instance later)
        self._register_category("Delegation", [
            SubAgentTool(),
            DelegationInfoTool()
        ])
    ...
    def list_tools_by_category(self):
        ...
        categories = {
            ...
            "Conversation Management": ["compact_conversation", "conversation_stats"],
            "Delegation": ["delegate_task", "get_delegation_info"]
        }
...
def get_tool_instance(name):
    """Get tool instance by name"""
    return _tool_manager.get_tool_instance(name)


def get_tool_instance(name):
    """Get tool instance by name"""
    return _tool_manager._registry.get_tool_instance(name)
