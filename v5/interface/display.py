...
class DisplayManager:
    ...
    def show_statistics(self, stats_data):
        ...
        delegation_stats = stats_data.get('delegation_stats', {})
        if delegation_stats.get('total_delegations', 0) > 0:
            print(f"   • Total delegations: {delegation_stats['total_delegations']}")
            print(f"   • Delegation success rate: {delegation_stats['success_rate']:.1f}% "
                  f"(✓ {delegation_stats['successful_delegations']}, "
                  f"✗ {delegation_stats['failed_delegations']})")
            if delegation_stats.get('total_delegation_time', 0) > 0:
                print(f"   • Total delegation time: {delegation_stats['total_delegation_time']:.2f}s")

            agent_type_usage = delegation_stats.get('agent_type_usage', {})
            if agent_type_usage:
                print("   • Sub-agent usage:")
                for agent_type, count in sorted(agent_type_usage.items()):
