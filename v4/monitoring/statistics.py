...
class StatisticsCollector:
    ...
    def record_delegation_started(self, agent_type, task, agent_id):
        """Record start of delegation"""
        ...
        self.delegation_statistics['total_delegations'] += 1
        self.delegation_statistics['agent_type_usage'][agent_type] = \
            self.delegation_statistics['agent_type_usage'].get(agent_type, 0) + 1

    def record_delegation_completed(self, agent_type, agent_id, success, duration):
        """Record completion of delegation"""
        self.delegation_events.append({
        ...
        if success:
            self.delegation_statistics['successful_delegations'] += 1
        else:
            self.delegation_statistics['failed_delegations'] += 1

        self.delegation_statistics['total_delegation_time'] += duration
    ...
    def _show_delegation_stats(self):
        """Show delegation statistics"""
        if self.delegation_statistics['total_delegations'] > 0:
            success_rate = (self.delegation_statistics['successful_delegations'] /
                          self.delegation_statistics['total_delegations']) * 100
            print(f"   • Total delegations: {self.delegation_statistics['total_delegations']}")
            print(f"   • Success rate: {success_rate:.1f}% "
                  f"(✓ {self.delegation_statistics['successful_delegations']}, "
                  f"✗ {self.delegation_statistics['failed_delegations']})")
            print(f"   • Total delegation time: {self.delegation_statistics['total_delegation_time']:.2f}s")

            if self.delegation_statistics['agent_type_usage']:
                print("   • Sub-agent usage:")
                for agent_type, count in sorted(self.delegation_statistics['agent_type_usage'].items()):
                    print(f"     - {agent_type}: {count} times")
    ...
    def get_statistics_dict(self, messages=None, compressor=None, tool_executor=None):
        ...
        stats_data = {
            ...
            'delegation_stats': {
                'total_delegations': self.delegation_statistics['total_delegations'],
                'successful_delegations': self.delegation_statistics['successful_delegations'],
                'failed_delegations': self.delegation_statistics['failed_delegations'],
                'success_rate': (self.delegation_statistics['successful_delegations'] /
                               self.delegation_statistics['total_delegations'] * 100)
                               if self.delegation_statistics['total_delegations'] > 0 else 0,
                'agent_type_usage': self.delegation_statistics['agent_type_usage'].copy(),
                'total_delegation_time': self.delegation_statistics['total_delegation_time'],
                'delegation_events': self.delegation_events.copy()
            }
        }
